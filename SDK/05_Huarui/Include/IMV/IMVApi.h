...
/// \remarks
/// 修改当前进程的日志等级，重启程序后日志等级为日志配置文件中的日志等级
/// \~english
/// \brief Modify the log level.
/// \param logLevel [IN] log level
/// \return Success, return IMV_OK. Failure, return error code
/// \remarks
/// Modify the log level of the current process. After restarting the program, the log level is the log level in the log configuration file.
IMV_API int IMV_CALL IMV_ModifyLogLevel(IN IMV_EMVLOG_LEVEL logLevel);
...
/// \~chinese
/// \brief 设备本地升级(适用于Gige和USB)
/// \param handle [IN]  设备句柄
/// \param pstUpGradeParam [IN]  设备升级参数结构体	
/// \return 成功，返回IMV_OK；错误，返回错误码
/// \remarks 
/// 通过该接口可以将升级固件文件发送给设备进行升级。
/// 该接口需要等待升级固件文件成功传给设备端之后再返回，响应时间可能较长。
/// \~english
/// \brief Device Local Upgrade(For Gige and USB)
/// \param handle [IN]  Device handle
/// \param pstUpGradeParam [IN]  Device upgrade parameter struct		
/// \return Success, return IMV_OK. Failure, return error code
/// \remarks 
/// Call this API to send the upgrade firmware to the device for upgrade.
/// This API will wait for return until the upgrade firmware is sent to the device, this response may take a long time.
IMV_API int IMV_CALL IMV_LocalUpgrade(IN IMV_HANDLE handle, IN IMV_UpGradeParam* pstUpGradeParam);

/// \~chinese
/// \brief 获取升级进度(适用于Gige和USB)
/// \param handle [IN]  设备句柄
/// \param pProcess [IN_OUT]  进度接收地址	
/// \return 成功，返回IMV_OK；错误，返回错误码
/// \~english
/// \brief Get Upgrade Progress(For Gige and USB)
/// \param handle [IN]  Device handle
/// \param pProcess [IN_OUT]  Progress receiving address		
/// \return Success, return IMV_OK. Failure, return error code
IMV_API int IMV_CALL IMV_GetUpgradeProcess(IN IMV_HANDLE handle, IN_OUT unsigned short* pProcess);

/// \~chinese
/// \brief 加载ISP配置
/// \param handle [IN]  设备句柄
/// \param pFullFileName [IN]  ISP配置（xml）文件全名（含路径）如：D:\\ispConfig.xml	
/// \return 成功，返回IMV_OK；错误，返回错误码
/// \~english
/// \brief Load ISP Configuration
/// \param handle [IN]  Device handle
/// \param pFullFileName [IN]  The full path name of the ISP configuration file(xml). e.g. D:\\ispConfig.xml	
/// \return Success, return IMV_OK. Failure, return error code
IMV_API int IMV_CALL IMV_LoadISPConfig(IN IMV_HANDLE handle, IN const char* pFullFileName);
#ifdef __cplusplus
}
#endif 
