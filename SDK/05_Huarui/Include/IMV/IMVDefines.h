...
#define IMV_LOADXML_ERROR			-125		///< \~chinese 加载xml错误							\~english Error loading xml
#define IMV_OPEN_STREAMCHN_ERROR	-126		///< \~chinese 打开流通道错误						\~english Error opening stream channel
#define IMV_OPEN_CONTROLCHN_ERROR	-127		///< \~chinese 打开控制通道错误						\~english Error opening control channel
#define IMV_DRIVER_NOT_INSTALLED	-128		///< \~chinese 驱动未安装							\~english Driver not installed
#define IMV_CALL_DRIVER_ERROR		-129		///< \~chinese 调用驱动接口失败						\~english Failed to call driver interface
#define IMV_IMAGE_SIZE_ERROR		-130		///< \~chinese 图片大小异常							\~english Image size is abnormal
...
{
	demosaicNearestNeighbor,					///< \~chinese 快速(最近邻)		\~english fast(Nearest neighbor)
	demosaicBilinear,							///< \~chinese 均衡(双线性)		\~english balance(Bilinear)
	demosaicEdgeSensing,						///< \~chinese 最优(边缘检测)	\~english best(Edge sensing)
...
#define IMV_MSG_EVENT_ID_EXPOSURE_START		 		0x900C
...
#define IMV_MSG_EVENT_ID_AUTO_FOCUS_END				0x9011
#define IMV_MSG_EVENT_ID_AUTO_LIGHT_END				0x9012

/// \~chinese
/// \brief 消息通道事件ID列表 -- 旧的曝光开始事件宏定义名称，后续将弃用
/// \~english
/// \brief Message channel event id list -- Old exposure start event macro definition name, deprecated
...
/// \~english
/// \brief Log level
typedef enum _IMV_EMVLOG_LEVEL
{
	IMV_LOG_LEVEL_NOLOG = 0,				///< \~chinese 无日志										\~english NO LOG		
	IMV_LOG_LEVEL_INFO  = 600,				///< \~chinese INFO等级										\~english INFO Level
	IMV_LOG_LEVEL_DEBUG = 700,				///< \~chinese DEBUG等级									\~english DEBUG Level
}IMV_EMVLOG_LEVEL;

/// \~chinese
/// \brief 设备升级结构体
/// \~english
/// \brief Device upgrade structure
typedef struct _IMV_UpGradeParam
{	
	bool				bForcedUpgrade;		///< [IN]   \~chinese 是否强制升级							\~english Force upgrade
	char*				pFullFilePath;		///< [IN]   \~chinese 固件路径								\~english Firmware path										
	unsigned int        nReserved[12];		///<	    \~chinese 预留									\~english Reserved							
}IMV_UpGradeParam;
